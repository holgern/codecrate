...
from .parse import module_name_for, parse_defs
...
def pack_repo(
    ...
    for path in files:
        ...
        defs = parse_defs(path=path, root=root, text=text)
    ...
    if not dedupe:
        ...
        for d in all_defs:
            ...
            remapped.append(replace(d, id=cid))
    ...
    pack = PackResult(root=root, files=filepacks, defs=all_defs)
